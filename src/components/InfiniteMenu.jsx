...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      // Continuous Slow Auto-Rotation when not dragging
      const autoAxis = vec3.fromValues(0.2, 1.0, 0.1);
      vec3.normalize(autoAxis, autoAxis);
      const autoAngle = 0.0015 * timeScale; // Slow and elegant spin
      const autoQuat = quat.setAxisAngle(quat.create(), autoAxis, autoAngle);
...
class InfiniteGridMenu {
  ...
  start() {
    ...
    const loop = (time = 0) => {
      if (!this.running) return;
      this.#deltaTime = Math.max(0, Math.min(32, time - this.#time));
      this.#time = time;
  ...
  #render() {
    ...
    // Alpha support for transparent viewport backgrounds
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      ...
      if (nearestVertexIndex !== undefined && !isNaN(nearestVertexIndex)) {
        const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
        if (!isNaN(itemIndex) && itemIndex !== this.activeItemIndex) {
...
export default function InfiniteMenu({ items = [], scale = 1.0, isPlaying = true }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        ...
        () => {}, // No-op active item change
        () => {}, // No-op movement change
        null,
        scale
      );
  ...
  // Manage visibility play/pause loop on isPlaying state change
  useEffect(() => {
